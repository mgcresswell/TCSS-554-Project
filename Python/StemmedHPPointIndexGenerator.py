...
songs = db.GetMetroSongs(connection, "where songs.StemmedLyrics is not null;")
repIndex = []

for index, row in songs.iterrows(): 
    tokens = Tokenize(row)
    repIndex.append([row['SongId'],GroupTokens(tokens)])

newconnection = db.GetCloudConnection()
with newconnection.cursor() as cursor:
       for i in range(len(repIndex)):
           sql = "INSERT INTO StemmedMetroLyricHIndex (SongId,HIndex) VALUES (%s,%s) "
